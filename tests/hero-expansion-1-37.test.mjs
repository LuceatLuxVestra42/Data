...
for (const spec of [
  {id:1, name:'马修', ko:'매튜', hpRaw:500, hpPct:5, assetId:1012, assetSha:'866a85af4b320bd02e943445a91dec52c34a00d956d21064aa2d3b80cdde3f64'},
  {id:37, name:'古巨拉', ko:'쥬그라', hpRaw:1500, hpPct:15, assetId:1024, assetSha:'c91022024571d103dee3c49be81fea9251e41662cb73fbf7e511f149be0af2f2'}
]) {
  ...
  test(`Hero ${spec.id} localization and SP artwork remain presentation-only`, () => {
    const localization = readJson(`data/hero/${spec.id}/localization.ko.v1.json`);
    const asset = readJson(`data/hero/${spec.id}/asset.sp-artwork.v1.json`);
    ...
    assert.equal(asset.charImageId, spec.assetId);
...
test('aggregate Hero consumer is deterministic and pinned', () => {
  ...
  assert.equal(digest, '0d8c3177703b83a042b7a053d83072914f0691f10f358ad1843c3284054e9963');
